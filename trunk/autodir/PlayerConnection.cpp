...
// Included for access to the GLWidgetSubview class
#include "GLWidget.h"

// Included for access to the store/thaw-style methods
...
void PlayerConnection::requestVideoInputList()
{
	...
	sendCommand(QVariantList() << "cmd" << Server_ListVideoInputs);
}
...
void PlayerConnection::receivedMap(QVariantMap map)
{
	...
	if(cmd == Server_CurrentPlaylistItemChanged ||
	   cmd == Server_PlaylistTimeChanged)
	{
		...
		if(cmd == Server_PlaylistTimeChanged)
		{
