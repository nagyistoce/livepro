...
QString MD5::md5sum(QString val)
{
    ...
    QtMD5::MD5_CTX context;
    QtMD5::MD5Init(&context);
    QtMD5::MD5Update(&context, (unsigned char *)val.toAscii().data(), val.size());
    QtMD5::MD5Final(digest, &context);
    QString output, tmp;
    for (int i = 0; i < 16; ++i)
...
QString MD5::md5sum(QByteArray & ba)
{
    ...
    QtMD5::MD5_CTX context;
    QtMD5::MD5Init(&context);
    QtMD5::MD5Update(&context, (unsigned char *)ba.data(), ba.length());
    QtMD5::MD5Final(digest, &context);
    QString output, tmp;
    for (int i = 0; i < 16; ++i)
        output += tmp.sprintf("%02x", digest[i]);
